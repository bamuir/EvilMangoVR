...
public class UnitySerialPort : MonoBehaviour
{
    ...
    public static string rawData = "Ready";
    ...
    void Update()
    {
        if (UI_commandDisplay != null && RawData != "")
        {
            if (!RawData.Contains("fail"))
            {
                UI_commandDisplay.text = RawData;
            }
            //Debug.Log("SerialPort RawData: " + RawData);
        }
        RawData = "";
    ...
    public void SendSerialDataAsLine(string data)
    {
        if (SerialPort != null)
        {
            ...
            print("Sent data: " + data);  SerialPort.WriteLine(data);
        }
